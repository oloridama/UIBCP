...
use crate::proto::uibc::v1::{
    ...
    // NOTE: Ics23Proof might be missing if your proto compilation output differs
    Ics23Proof,
};
...
pub fn process_message(
    ...
    // NOTE: is_valid() is a custom method you need to implement on UniversalMessage
    if !message.is_valid() {
    ...
    let zk_req = match &message.proof_requirement {
        Some(req) => match &req.requirement {
            Some(Requirement::ZkProof(r)) => Some(r.clone()),
            _ => None,
        },
        None => None,
    };
...
pub fn handle_message_and_submit(
    ...
    // NOTE: canonical_encode() is a custom method you need to implement
    println!("Canonical bytes = {:?}", message.canonical_encode());

    if let Some(fees) = &message.fees {
        // Adjust according to how Fee is defined in proto
        if let Some(total_fee) = &fees.total_fee {
            println!("Total fee: {} {}", total_fee.amount, total_fee.denom);
        }
    }
