...
use relayer::zk::circuit::{Ics23Prover, PublicInputs, Ics23Air, bytes_to_field, POSEIDON_WIDTH, POSEIDON_FULL_ROUNDS, POSEIDON_PARTIAL_ROUNDS, MERKLE_LEVELS, TRACE_WIDTH, TOTAL_STEPS};
...
pub fn generate_zkp(message: uibc::v1::UniversalMessage, proof: uibc::v1::Ics23Proof, root: [BaseElement; 4], zk_req: Option<uibc::v1::ZkProofRequirement>, ibc_data: Option<uibc::ibc::v1::IbcCompatibilityData>, evm_ext: Option<uibc::ibc::extensions::EVMExtension>) -> Result<Vec<u8>, &'static str> {
    let air = Ics23Air {
        ...
        pub_inputs: PublicInputs {
            ...
            gas_limit: evm_ext.map_or(BaseElement::ZERO, |e| BaseElement::new(e.gas_limit % MODULUS as u64)),
            max_fee_per_gas: evm_ext.map_or(BaseElement::ZERO, |e| BaseElement::new(e.max_fee_per_gas % MODULUS as u64)),
        },
    };
    let prover = Ics23Prover { air, options: ProofOptions::default() };
    let trace = prover.build_trace(message, proof, zk_req, ibc_data, evm_ext);
    Ok(prover.prove(trace))
}
...
pub fn process_message(message: uibc::v1::UniversalMessage, proof: uibc::v1::Ics23Proof, root: [BaseElement; 4]) -> Result<Vec<u8>, &'static str> {
    ...
    if message.message_id.is_empty() {
        return Err("Message has no message_id");
    }
    if proof.siblings.len() != MERKLE_LEVELS || proof.path.len() != MERKLE_LEVELS {
